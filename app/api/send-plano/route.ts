...
function drawFooter(doc: LayoutDoc, page: PDFPage) {
  ...
  const scriptFont = doc.fontScript || doc.fontBold;

  // Linha 1: "Nutricionista: {nome}" em GreatVibes 18pt
  const nutriLineSize = doc.fontScript ? 18 : 13;
  const fullNutriText = nomeBase ? `Nutricionista: ${nomeBase}` : "Nutricionista:";
  const nutriTextWidth = Math.min(
    scriptFont.widthOfTextAtSize(fullNutriText, nutriLineSize),
    PAGE_WIDTH - PAGE_MARGIN_X * 2 - 70
  );

  page.drawText(fullNutriText, {
    x: PAGE_MARGIN_X,
    y: footerY + 12,
    size: nutriLineSize,
    font: scriptFont,
  ...
  // Linha acompanhando apenas o texto do nutricionista
  page.drawLine({
    start: { x: PAGE_MARGIN_X, y: footerY + 10 },
    end: { x: PAGE_MARGIN_X + nutriTextWidth, y: footerY + 10 },
    thickness: 0.8,
    color: rgb(0.2, 0.2, 0.2),
  });

  // Linha 2: CRN em GreatVibes 16pt (2pt menos que o nome)
  const crnLineSize = doc.fontScript ? 16 : 11;
