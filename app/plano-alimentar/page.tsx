...
export default function PlanoAlimentarPage() {
  ...
  const [sex, _setSex] = useState<'Feminino' | 'Masculino'>('Feminino')
  void _setStrategy; void _setTotalKcal; void _setSex
  ...
  // ==================== RENDER ====================
  return (
    <div style={{ fontFamily: "'Inter', 'Segoe UI', sans-serif", background: 'linear-gradient(135deg, #f5f7fa 0%, #e4e9f0 100%)', color: '#2d3748', minHeight: '100vh' }}>
      {/* Sidebar */}
      <div
        style={{
          position: 'fixed',
          ...
          width: 160,
          height: '100vh',
          background: 'linear-gradient(180deg, #0f2847 0%, #1a365d 50%, #0f2847 100%)',
          color: '#fff',
          paddingTop: 18,
          zIndex: 10,
      ...
      >
        <div style={{ textAlign: 'center', padding: '8px 12px 20px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <h2 style={{ fontSize: 20, fontWeight: 800, background: 'linear-gradient(135deg, #63b3ed, #4fd1c5)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>NutriPlan</h2>
          <small style={{ fontSize: 9, color: '#90cdf4', letterSpacing: 1, textTransform: 'uppercase' }}>Sistema de Nutrição</small>
        </div>
        <nav style={{ marginTop: 12 }}>
          {[
            { href: '/plano-alimentar', icon: '📋', label: 'Plano Alimentar', active: true },
            { href: '/taxa-metabolica', icon: '🔥', label: 'Taxa Metabólica', active: false },
            { href: '#', icon: '👥', label: 'Pacientes', active: false },
            { href: '#', icon: '🍎', label: 'Alimentos', active: false },
            { href: '#', icon: '📊', label: 'Relatórios', active: false },
            { href: '#', icon: '📅', label: 'Agenda', active: false },
            { href: '#', icon: '⚙️', label: 'Configurações', active: false },
          ].map((item) => (
            <a
              key={item.label}
              href={item.href}
              style={{
                ...
                alignItems: 'center',
                padding: '11px 18px',
                color: item.active ? '#fff' : '#a0aec0',
                textDecoration: 'none',
                fontSize: 12,
                fontWeight: item.active ? 600 : 500,
                gap: 8,
                borderLeft: item.active ? '3px solid #63b3ed' : '3px solid transparent',
                background: item.active ? 'rgba(99,179,237,0.15)' : 'transparent',
              }}
            >
      ...
      {/* Main */}
      <div style={{ marginLeft: 160, padding: '20px 24px', minHeight: '100vh' }}>
        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1a365d' }}>Plano Alimentar</h1>
          <div
            style={{
              display: 'flex',
              alignItems: 'center',
          ...
          >
            ...
            <div>
              <strong style={{ fontSize: 13, color: '#1a365d', display: 'block', fontWeight: 700 }}>Maria Silva</strong>
              <span style={{ fontSize: 11, color: '#718096' }}>{sex} · 32 anos · 65kg</span>
            </div>
          </div>
        </div>
