...
// ---------- Biotipo corporal ----------
// Regras FFMI x BF% -> imagem em public/images/avaliacao/
// (não altera nenhum outro trecho do layout)
function biotipoFileName(sexo: "M" | "F", ffmi: number, bfPct: number): string {
  if (sexo === "F") {
    if (ffmi >= 19.0) return bfPct <= 20 ? "fem-frente-3.png" : "fem-frente-4.png";
    if (ffmi < 14.5) return bfPct > 26 ? "fem-frente-2.png" : "fem-frente-6.png";
    return bfPct <= 26 ? "fem-frente-1.png" : "fem-frente-5.png";
  }
  if (ffmi >= 21.5) return bfPct <= 12 ? "masc-frente-3.png" : "masc-frente-4.png";
  if (ffmi < 17.5) return bfPct > 16 ? "masc-frente-2.png" : "masc-frente-6.png";
  return bfPct <= 16 ? "masc-frente-1.png" : "masc-frente-5.png";
}
...
const styles = StyleSheet.create({
  ...
  // Cabeçalho — idêntico ao PDF de Plano/Orientações (mock imagem 1):
  // logo 60 à esquerda, nome do paciente em Helvetica-Bold 15pt,
  // duas linhas de metadados em 10pt bold preto, régua fina escura embaixo.
  header: { flexDirection: "row", alignItems: "center" },
  logoBox: { width: 66, alignItems: "center", marginRight: 10 },
  ...
  rule: { height: 1, backgroundColor: RULE, marginTop: 8, marginBottom: 12 },
  ...
  // Ajuste: cards do bloco topo com altura fixa reduzida — colam com a régua
  // do bloco médio (sem o "vão" branco aleatório que aparecia embaixo).
  topRow: { flexDirection: "row", justifyContent: "space-between", marginBottom: 8 },
  topLeft: { width: "56%", height: 176 },
  topRight: { width: "42%", height: 176, alignItems: "center", justifyContent: "center" },
  ...
  bodyPlaceholder: { width: 132, height: 150, marginTop: 4 },

  // Ajuste 1/4: card de evolução exibido na 1ª avaliação (sem comparação)
  // Silueta homem+mulher à esquerda, texto ao lado — ambos contidos DENTRO
  // do card (sem estourar as bordas). Caixa verde da nota também recolhida
  // para caber no retângulo, com quebra de linha natural.
  evoInfoRow: { flexDirection: "row", alignItems: "center", marginTop: 2 },
  evoFigure: { width: 54, height: 62, objectFit: "contain", marginRight: 6 },
  evoFigureSvg: { width: 54, height: 62, marginRight: 6 },
  evoInfoTextWrap: { flexGrow: 1, flexShrink: 1, paddingRight: 2 },
  evoInfoText: { fontSize: 6.8, color: "#333", lineHeight: 1.3 },
  evoNoteBox: {
  ...
  evoNoteIcon: { width: 12, height: 12, marginRight: 5 },
  evoNoteTextWrap: { flexGrow: 1, flexShrink: 1, paddingRight: 2 },
  evoNoteText: { fontSize: 6.4, color: "#2e4630", lineHeight: 1.3 },
  ...
  // Assinatura — mesmo tamanho/espaçamento do PDF de Plano/Orientações
  // (mock imagem 1): "Nutricionista: {nome}" GreatVibes 18, linha fina
  // acompanhando apenas o texto, CRN GreatVibes 16, logo 42 opacidade 0.15
  ...
  signLine: { height: 0.8, backgroundColor: "#333", marginTop: 1 },
...
// Silueta homem + mulher — usa a imagem `public/images/avaliacao/silueta.png`.
// Se o arquivo não existir por algum motivo, cai num SVG básico como fallback
// (mantendo o layout do card intacto).
function SilhuetaSvg() {
  const VERDE = "#8fae7f";
  return (
    <Svg width={54} height={62} viewBox="0 0 100 100" style={styles.evoFigureSvg}>
      <Circle cx={35} cy={20} r={9} fill={VERDE} />
      <Rect x={26} y={30} width={18} height={38} rx={6} fill={VERDE} />
      <Circle cx={68} cy={22} r={8} fill={VERDE} />
      <Rect x={60} y={32} width={16} height={36} rx={7} fill={VERDE} />
    </Svg>
  );
}
...
// Ajuste 1/4: card informativo de evolução (silhueta homem+mulher + texto),
// exibido no lugar do gráfico vazio quando é a primeira avaliação / sem
// comparação. Silueta vem da imagem `public/images/avaliacao/silueta.png`.
function EvolucaoInfoCard({ siluetaSrc }: { siluetaSrc?: string | null }) {
  return (
    <View>
      <View style={styles.evoInfoRow}>
        {siluetaSrc ? (
          <Image src={siluetaSrc} style={styles.evoFigure} />
        ) : (
          <SilhuetaSvg />
        )}
...
export function AvaliacaoPdfDocument({ paciente, dados, nutricionista }: PdfProps) {
  ...
  const silueta = fileToDataUri("/images/avaliacao/silueta.png");

  // Biotipo corporal — FFMI = Massa Livre de Gordura / altura² (fallback
  // calculado quando dados.ffmi não vier preenchido) combinado com o BF%.
  const alturaM = Number(paciente.altura_cm) / 100;
  const ffmiVal = Number.isFinite(Number(dados.ffmi))
    ? Number(dados.ffmi)
    : (Number(dados.pesoKg) - Number(dados.massaGordaKg || 0)) / (alturaM * alturaM);
  const biotipoSrc = fileToDataUri(
    `/images/avaliacao/${biotipoFileName(paciente.sexo, ffmiVal, Number(dados.bfPct))}`
  );
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.topRow}>
          ...
          {/* Card direito — BIOTIPO CORPORAL (imagem conforme FFMI x BF%) */}
          <View style={[styles.card, styles.topRight]}>
            <Text style={styles.cardTitleCentered}>BIOTIPO CORPORAL</Text>
            {biotipoSrc ? (
              <Image src={biotipoSrc} style={styles.bodyPlaceholder} />
            ) : (
        ...
        <View style={styles.midRow}>
          ...
          <View style={styles.cardMid}>
            ...
            {showEvoCard ? (
              ...
              <EvolucaoInfoCard siluetaSrc={silueta} />
            )}
          </View>
        </View>
        ...
        {/* ============ ASSINATURA ============ */}
        {/* Mesmo padrão do PDF de Plano/Orientações (mock imagem 1):
            "Nutricionista: {nome}" GreatVibes 18 com linha acompanhando
            o texto, e CRN em GreatVibes 16 logo abaixo. */}
            ...
            <View
              style={[
                styles.signLine,
                {
                  width:
                    18 * 0.42 *
                    (`Nutricionista: ${nutricionista?.nome || "Nutricionista"}`).length,
                },
              ]}
            />
