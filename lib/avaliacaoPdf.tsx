...
function formatReferenceForPdf(value: string | null | undefined) {
  if (!value) return "—";
  // Extrai somente número/faixa e unidade. Assim "Adequado", "e" e
  // qualquer texto residual nunca chegam ao font do PDF.
  const match = value.match(
    /(\d+(?:[.,]\d+)?(?:\s*[–-]\s*\d+(?:[.,]\d+)?)?)\s*(kg\/m²|kg|%)/i,
  );
  if (!match) return value.trim() || "—";

  const numero = match[1].replace(/\s*([–-])\s*/g, "$1");
